...
const syncAllMetrics = async () => {
  try {
    ...
    const activeSubmissions = [
      { userId: "user1", contestId: "contest1", platform: "youtube", contentUrl: "https://youtube.com/watch?v=123" },
      { userId: "user2", contestId: "contest1", platform: "tiktok", contentUrl: "https://tiktok.com/@user/video/456" },
      {
        userId: "user3",
        contestId: "contest2",
        platform: "twitter",
    ...
    for (const submission of activeSubmissions) {
      try {
        ...
        console.log(`[v0] Synced metrics for ${submission.platform} submission:`, result)

        // Add delay to respect API rate limits
        await new Promise((resolve) => setTimeout(resolve, 1000))
      } catch (error) {
        console.error(`[v0] Failed to sync metrics for submission:`, submission, error)
      }
    }
...
const updateRankingsAndNotify = async (syncResults) => {
  ...
  for (const [contestId, results] of Object.entries(contestGroups)) {
    ...
    const sortedResults = results.sort((a, b) => b.result.engagementScore - a.result.engagementScore)
    ...
    for (let i = 0; i < sortedResults.length; i++) {
      ...
      if (newRank !== previousRank) {
        console.log(`[v0] Rank change for user ${result.userId}: ${previousRank} → ${newRank}`)
...
syncAllMetrics()
  .then((results) => {
    console.log("[v0] Sync completed successfully:", results.length, "submissions processed")
  })
  .catch((error) => {
