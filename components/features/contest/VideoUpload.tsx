...
export default function VideoUpload({ onVideoChange, className }: VideoUploadProps) {
  const [uploadType, setUploadType] = useState<'file' | 'youtube_link' | 'none'>('none');
  ...
  const ALLOWED_TYPES = ['video/mp4', 'video/avi', 'video/mov', 'video/wmv', 'video/flv', 'video/webm'];
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setError(`File size exceeds 100MB limit. Please upload to YouTube and provide the link instead.`);
  ...
  return (
    <Card className={`border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white ${className}`}>
      ...
      <CardContent className="space-y-6 p-6">
        ...
        {uploadType === 'youtube_link' && (
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="youtube-link" className="font-black uppercase text-black">
                YouTube Video Link
              </Label>
        ...
        <Alert className="border-4 border-black bg-blue-500 text-white">
          ...
          <AlertDescription className="font-bold">
            <strong>File Size Limit:</strong> Videos larger than 100MB should be uploaded to YouTube first, then provide the link here.
          </AlertDescription>
        </Alert>
      </CardContent>
    </Card>
