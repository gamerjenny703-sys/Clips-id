...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!amount || !bankDetails || !bankDetails.bankCode || !bankDetails.accountNumber) {
    ...
    if (irisResponse.payouts && irisResponse.payouts[0].status === 'queued') {
        ...
        const { error: decrementError } = await supabase.rpc('decrement_balance_and_log_payout', {
        ...
        if (decrementError) {
            ...
            console.error("CRITICAL: Failed to decrement balance after payout creation:", decrementError);
            // Untuk sekarang, kita kembalikan error
            throw new Error("Payout created but failed to update user balance.");
        }

        return NextResponse.json({ status: "success", message: "Payout request submitted successfully!", data: irisResponse });
    } else {
        // Jika API Midtrans mengembalikan error
        throw new Error(`Midtrans Iris rejected the payout: ${JSON.stringify(irisResponse)}`);
    }
